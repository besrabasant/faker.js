...
function Random (faker, seed) {
   ...
   * hindiNumeric
   *
   * @method faker.random.hindiNumber
   * @param {number} count defaults to random
   */
  this.hindiNumber = function hindiNumber(count) {
    if (typeof count === "undefined") {
      count = faker.random.number({min:1, max:7})
    }

    var wholeString = "";
    for(var i = 0; i < count; i++) {
      wholeString += faker.random.arrayElement(["०","१","२","३","४","५","६","७","८","९"]);
    }
